			...
			if ( is_dir( $plugin_root.'/'.$file ) ) {
				...
				if ( $plugins_subdir ) {
					while (($subfile = readdir( $plugins_subdir ) ) !== false ) {
						if ( substr($subfile, 0, 1) == '.' )
							continue;
						if(is_dir($plugin_root . '/' . "$file/$subfile")){
							$plugins_subdir_subdir = @ opendir($plugin_root . '/' . $file.'/'.$subfile);
							if($plugins_subdir_subdir){
								while(($subfile_file = readdir($plugins_subdir_subdir)) !== false){
									if (substr($subfile_file, 0, 1) == '.') {
										continue;
									}
									if(substr($subfile_file,-4) == '.php'){
										$plugin_files[] = "$file/$subfile/$subfile_file";
									}
								}
							}
